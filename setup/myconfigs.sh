...
# Git
git config --global user.email "47799153+GitDoVitor@users.noreply.github.com
" && git config --global user.name "gitdovitor"

# Dirs
mkdir -p $HOME/{Images,Downloads/Arch,Videos,Projects,.ssh,Github} &> /dev/null

# My Tokens
# tokens() {
#     sudo rm -rf $HOME/GitHub/mytokens
#     sudo rm -rf $HOME/.ssh/id_*
#     echo -e "\n${BOL_GRE}Será necessário autenticar a conta do ${BOL_MAG}GITHUB ${BOL_GRE}manualmente...${END}\n"
...
for repo in ${repos[@]}; do
  ...
  # [[ $repo == "myhistory" ]] && gitHost="github"
  [[ $repo == "mywifi" ]] && gitHost="github"
